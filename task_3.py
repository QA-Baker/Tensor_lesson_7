...
class PublicTransport:
    ...
    def info(self):
        ...
        return (f"Марка: {self.brand}, Цвет: {self.color}, "
                f"Год выпуска: {self.year}, Мощность двигателя: {self._engine_power}")
...
class Bus(PublicTransport):
    def __init__(self, brand, engine_power, year, color, max_speed, passengers, park, fare):
        ...
        self.__park = park  # приватный атрибут
        self._fare = fare  # защищённый атрибут

    @property
    def park(self):
        # Возвращаем значение приватного атрибута park
        return self.__park

    @park.setter
    def park(self, value):
        # Проверяем, что номер парка в диапазоне 1000–9999
...
class Tram(PublicTransport):
    ...
    def how_long(self):
        ...
        return round(self.max_speed / (4 * self.path), 2)
